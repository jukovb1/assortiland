...
{		
	...
	$md5_new_file = trim(md5_file($file));
	$file_mass_add=array();
	$flag=false;
	foreach ($format_mass as $key=>$value){
		if($file_name_arr[1]==$value){
			
		 $file_mass_add['old_file_name']=$file_name;
		 $file_mass_add['new_file_name']=$md5_new_file.'.'.$value;
		 $file_mass_add['date']=date('Y-m-d H:m:s');
			//$GLOBALS['db']->insert_array_to_table('files',$file_mass_add);
			echo '<a href="'.$files_path.'files/'.$file_name.'" id="#####'.$file_name.'#####" rel="'.$file_name.'" class="redactor_file_link redactor_file_ico_'.$file_ico.'">'.$file_name.'</a>';
			$flag=true;		
		}
