...
<div class="drv-top-wrapper">
    <div class="drv-common-sort">
        <select id="sort" name="sort" class="wid100">
            ...
            <option value="1"><?=lang_text('{user_action_show}')?></option>
            <option value="0"><?=lang_text('{user_action_hide}')?></option>
    ...
    <?
        ...
        foreach ($content_by_group as $content_id => $content_data) {
            ...
            <tr class="<?=$hide_class?> tr_<?=$content_id?>">
                <td class="sh2_<?=$content_id?>" data-cont_id="<?=$content_id?>" data-cur_status="<?=$content_data['user_status']?>"><input type="checkbox" name="chbx[<?=$content_id?>]" value="1" class="check_item niceCheck" id="chbx_<?=$content_id?>" tabindex="1" /></td>
                <?
