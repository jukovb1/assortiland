...
function cuSelRefresh(a) {
    var b = a.refreshEl.split(","),
        ...
        d;
    for (d = 0; d < c; d++) {
        var e = $(b[d]).parents(".cusel").find(".cusel-scroll-wrap").eq(0);
    ...
    function b() {
        ...
        $("html").click(function (a) {
            ...
            if ((d.indexOf("cuselText") != -1 || d.indexOf("cuselFrameRight") != -1) && b.parent().prop("class").indexOf("classDisCusel") == -1) {
                ...
                b.parents(".cusel-scroll-wrap").find(".cuselActive").eq(0).removeAttr('selected').removeClass("cuselActive").end().parents(".cusel-scroll-wrap").next().val(f).end().prev().text(b.text()).end().css("display", "none").parent(".cusel").removeClass("cuselOpen");
                ...
                $(".cusel-scroll-wrap").css("display", "none").parent(".cusel").removeClass("cuselOpen")
            }
        });
        $(".cusel").unbind("keydown");
        $(".cusel").keydown(function (a) {
            var b, c;
            if (window.event) b = window.event.keyCode;
            else if (a) b = a.which;
            if (b == null || b == 0 || b == 9) return true;
            if ($(this).prop("class").indexOf("classDisCusel") != -1) return false;
            if (b == 40) {
                ...
                var f = e.next();
                if (f.is("span")) {
                    $(this).find(".cuselText").eq(0).text(f.text());
        ...
        $(".cusel").keypress(function (b) {
            function g() {
                ...
                for (g = 0; g < f; g++) {
                    ...
                    if (i) {
                        $(e).find(".cuselOptHover").removeClass("cuselOptHover").end().find("span").eq(g).addClass("cuselOptHover").end().end().find(".cuselText").eq(0).text(d.eq(g).text());
            ...
            $(this).data("timer", f);
            if ($.browser.opera && window.event.keyCode != 9) return false
        })
    }
    $(a.changedEl).each(function (c) {
        var d = $(this),
            e = d.outerWidth(),
            f = d.prop("class"),
        ...
        var w = '<div class="cusel ' + f + " " + classDisCusel + '"' + " id=cuselFrame-" + g + ' style="width:' + e + 'px"' + ' tabindex="' + p + '"' + ">" + '<div class="cuselFrameRight"></div>' + '<div class="cuselText">' + l + "</div>" + '<div class="cusel-scroll-wrap"><div class="cusel-scroll-pane" id="cusel-scroll-' + g + '">' + u + "</div></div>" + '<input ' + req + ' type="hidden" id="' + g + '" name="' + h + '" value="' + j + '" />' + "</div>";
        d.replaceWith(w);
        if (o) $("#" + g).bind("change", o);
        if (s.length) {
            $.each(s, function (a, b) {
                $("#" + g).bind("change", b)
            })
        ...
        var A = $("#cusel-scroll-" + g).find("span[addTags]"),
            B = A.length;
