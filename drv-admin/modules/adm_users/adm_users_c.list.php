...
if (isset($_GET['id'])){
    ...
    $content_v = $_SERVER['DOCUMENT_ROOT']."/$adm_folder/modules/$_cur_area/{$_cur_area}_v.users_list.php";
    $count_items = adm_users_m::count_users();
    $page_nav  = page_navigation::get_page_by_num_for_adm($count_items,'пользователей',$num_of_rows_per_page);
    $content_by_group = adm_users_m::get_users_list($page_nav['nav_start_position'],$page_nav['nav_limit']);
}
