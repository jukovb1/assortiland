...
if (isset($_GET['id'])){
    ...
    if ($_GET['id']=='add'){
        ...
        $content_by_id_data['result_data']['group_id'] = 'new';
        $content_by_id_data['result_data']['group_short_name'] = '';
        $content_by_id_data['result_data']['group_full_name'] = '';
        $content_by_id_data['result_data']['group_description'] = '';
        $content_by_id_data['result_data']['group_parent_group'] = 0;

    } else {
        $content_id = intval($_GET['id']);
        $content_by_id_data = adm_products_m::get_products_group_by_id($content_id);
        //ash_debug($content_by_id_data);
    }

    if(!empty($_POST)) {
        $prep_post_data = adm_products_c::group_data_save_prepare($_POST);
        ...
        if (!$prep_post_data['result']){
            ...
            $save_result = adm_products_m::update_products_group($prep_post_data['result_data']);
...
$content_by_group_no_sort = adm_products_m::get_products_groups();
$content_by_group = resort_array_groups_data($content_by_group_no_sort,0);
